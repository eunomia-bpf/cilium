...
to networking complexity. Traditional networking models, in this context,
struggle with service discovery, network segmentation, policy enforcement,
...
* ``etcd``: Stores the local cluster state that is exposed to remote clusters
  ...
  persistent storage as it is rebuilt from kube-apiserver on startup.
